...
Some functions such as "rotate board" I had never done before in any langauge, so AI helped me to generate the correct code. I manually tested by dropping a few pieces in, then rotating the board and checking if the dimensions and pieces are still correct. This method of testing AI content was followed for many generated code. Some functions I knew the logic of what to do, and just asked AI to do this specifically to speed up the process. Others I knew conceptually what I wanted so simply described that and manually tested out the end result with various inputs

Often times the AI would assume certain things. One such example is when I asked it to create a function to drop the piece, and specified exactly what the parameters are and what the logic should be. I made it clear that the piece must be dropped, then the board must be rotated, and the piece dropped again. The AI, presumably using knowledge from classic Connect 4, did not rotate the board. I double checked the produced code and realised the mistake before implementing it into the final solution
